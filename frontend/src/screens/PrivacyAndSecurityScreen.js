...
import { createStyles } from "../styles/PrivacyAndSecurityScreen.styles";
...
const PrivacyAndSecurityScreen = ({ navigation }) => {
  ...
  const [request2faOtp, { isLoading: isRequestingOtp }] =
    useRequest2faOtpMutation();
  ...
  const sendOtpToSecondary = async () => {
    if (!secondaryEmail || !secondaryEmail.includes("@")) {
      showAlert({
        title: t("error"),
        message: t("valid_secondary_email_error"),
        type: "error",
      });
      return;
    }
    try {
      ...
      showAlert({
        title: t("success"),
        message: t("verification_code_sent") + secondaryEmail,
        type: "success",
      });
    } catch (err) {
      showAlert({
        title: t("failed") || t("error"),
        message: err.data?.detail || t("otp_failed_msg"),
        type: "error",
      });
    }
  };
  ...
  const verifyAndEnable2FA = async () => {
    ...
    try {
      ...
      showAlert({
        title: t("enable_2fa"),
        message: t("two_factor_active_msg"),
        type: "success",
      });
    } catch (err) {
      showAlert({
        title: t("invalid_code") || t("error"),
        message: t("incorrect_otp_msg"),
        type: "error",
      });
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <Text style={styles.introText}>{t("security_priority")}</Text>
