...
import { createStyles } from "../styles/ReportDetailsScreen.styles";
...
const ReportDetailsScreen = ({ route, navigation }) => {
  ...
  const handleShare = async () => {
    try {
      ...
      const reasoning = reportData.kbs_recommendation?.explainable_reasoning;
      const ruleLines = Array.isArray(reasoning)
        ? reasoning
            .map((r) => {
  ...
  const handleDownload = async () => {
    ...
    try {
      const pdfData = {
        ...
        result: reportData.kbs_recommendation?.risk_classification || t("unknown"),
        status: reportData.kbs_recommendation?.disease_detection || t("diagnosis"),
        ...
        details: {
          vitals: {
            body_temperature: reportData?.symptoms?.body_temperature,
            blood_pressure: reportData?.symptoms?.blood_pressure,
            heart_rate: reportData?.symptoms?.heart_rate,
          },
          labs: {
            platelet_count: reportData?.symptoms?.platelet_count,
            white_blood_cell_count: reportData?.symptoms?.white_blood_cell_count,
            hematocrit_level: reportData?.symptoms?.hematocrit_level,
          },
          warning_signs: {
            abdominal_pain: reportData?.symptoms?.abdominal_pain,
            persistent_vomiting: reportData?.symptoms?.persistent_vomiting,
            clinical_fluid_accumulation: reportData?.symptoms?.clinical_fluid_accumulation,
            mucosal_bleeding: reportData?.symptoms?.mucosal_bleeding,
            lethargy_or_restlessness: reportData?.symptoms?.lethargy_or_restlessness,
            liver_enlargement: reportData?.symptoms?.liver_enlargement,
            hematocrit_increases: reportData?.symptoms?.hematocrit_increases,
            platelet_decreases: reportData?.symptoms?.platelet_decreases,
          },
          severe_criteria: {
            severe_plasma_leakage: reportData?.symptoms?.severe_plasma_leakage,
            shock_dss: reportData?.symptoms?.shock_dss,
            respiratory_distress: reportData?.symptoms?.respiratory_distress,
            severe_bleeding: reportData?.symptoms?.severe_bleeding,
            ast_alt_1000: reportData?.symptoms?.ast_alt_1000,
            impaired_consciousness: reportData?.symptoms?.impaired_consciousness,
            heart_involvement: reportData?.symptoms?.heart_involvement,
          },
          home_care: {
            tolerates_oral_fluids: reportData?.symptoms?.tolerates_oral_fluids,
            urinating_regularly: reportData?.symptoms?.urinating_regularly,
            fever_drops: reportData?.symptoms?.fever_drops,
          },
        },
      };
  ...
  if (isError || !reportData) {
    return (
      ...
      >
        <Text style={{ color: colors.text }}>{t("report_load_error")}</Text>
        <TouchableOpacity
  ...
  const sections = [
    { key: "vital_signs", items: ["body_temperature", "blood_pressure", "heart_rate"] },
    { key: "laboratory_tests", items: ["platelet_count", "white_blood_cell_count", "hematocrit_level"] },
    {
      key: "warning_signs",
      items: [
        "abdominal_pain",
    ...
    { key: "phase_home_care_criteria", items: ["tolerates_oral_fluids", "urinating_regularly", "fever_drops"] },
  ];
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content}>
        ...
        <View style={styles.resultCard}>
          ...
          <Text style={styles.probabilityLabel}>{t("recorded_probability")}</Text>
          <View
            style={[
              styles.badge,
        ...
        <View style={styles.section}>
          ...
          <View style={styles.kvGrid}>
            <View style={styles.kvRow}>
              ...
              <Text style={styles.kvValue}>{`${probability}%`}</Text>
            </View>
            {exampleBenchmark && (
              <>
                <View style={styles.kvRow}>
        ...
        {sections.map((sec) => {
          const rows = sec.items
            .map((k) => ({ k, v: reportData?.symptoms?.[k] }))
