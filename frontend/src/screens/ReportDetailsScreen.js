...
import { ChevronLeft, Download, AlertCircle, CheckCircle, Info, List, Brain, FileText, Share2 } from 'lucide-react-native';
import { generateAndSharePDF, generateAndSavePDF } from '../utils/pdfGenerator';
import { useGetReportDetailQuery, useRecordActionMutation } from '../services/api';
...
const ReportDetailsScreen = ({ route, navigation }) => {
  ...
  const handleShare = async () => {
    try {
      const stageName = reportData.kbs_recommendation?.disease_detection || 'Dengue Analysis';
      ...
      const message = `📋 Dengue AI Health Report\n\n` +
        `Result: ${stageName}\n` +
        `Risk: ${riskLevel}\n` +
        `Probability: ${probValue}%\n\n` +
        `Recommendations:\n${recsList}\n\n` +
        `Generated by Dengue AI Health App`;

      const result = await Share.share({
        message,
        title: 'Dengue Health Report'
      });
