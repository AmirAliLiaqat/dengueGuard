...
const ResultScreen = ({ route, navigation }) => {
  ...
  const recommendation =
    diagnosis.clinical_recommendations || "Please consult a doctor.";
  ...
  const handleShare = async () => {
    try {
      const stageMessage =
        reportData.kbs_recommendation?.disease_detection || "Dengue Analysis";
      const riskLevel =
        reportData.kbs_recommendation?.risk_classification || "Unknown";
      ...
      <ScrollView style={styles.content}>
        {alertText && (
          <View
            style={[
              styles.section,
              { borderColor: riskColor, backgroundColor: riskColor + "1A" },
            ]}
          >
        ...
        <View style={styles.section}>
          ...
          {diagnosis.explainable_reasoning &&
          diagnosis.explainable_reasoning.length > 0 ? (
